...
#if defined(gsl_CONFIG_TRANSPARENT_NOT_NULL) && gsl_CONFIG_TRANSPARENT_NOT_NULL && \
...
#endif
#if 1 < \
  defined(gsl_CONFIG_UNENFORCED_CONTRACTS_ASSUME) + defined(gsl_CONFIG_UNENFORCED_CONTRACTS_ELIDE)
...
// AppleClang  7.0.0  __apple_build_version__ ==  7000172  gsl_COMPILER_APPLECLANG_VERSION ==  700
// (Xcode 7.0, 7.0.1)          (LLVM 3.7.0) AppleClang  7.0.0  __apple_build_version__ ==  7000176
// gsl_COMPILER_APPLECLANG_VERSION ==  700  (Xcode 7.1)                 (LLVM 3.7.0)
// AppleClang  7.0.2  __apple_build_version__ ==  7000181  gsl_COMPILER_APPLECLANG_VERSION ==  702
// (Xcode 7.2, 7.2.1)          (LLVM 3.7.0) AppleClang  7.3.0  __apple_build_version__ ==  7030029
// gsl_COMPILER_APPLECLANG_VERSION ==  730  (Xcode 7.3)                 (LLVM 3.8.0)
// AppleClang  7.3.0  __apple_build_version__ ==  7030031  gsl_COMPILER_APPLECLANG_VERSION ==  730
// (Xcode 7.3.1)               (LLVM 3.8.0) AppleClang  8.0.0  __apple_build_version__ ==  8000038
// gsl_COMPILER_APPLECLANG_VERSION ==  800  (Xcode 8.0)                 (LLVM 3.9.0)
// AppleClang  8.0.0  __apple_build_version__ ==  8000042  gsl_COMPILER_APPLECLANG_VERSION ==  800
// (Xcode 8.1, 8.2, 8.2.1)     (LLVM 3.9.0) AppleClang  8.1.0  __apple_build_version__ ==  8020038
// gsl_COMPILER_APPLECLANG_VERSION ==  810  (Xcode 8.3)                 (LLVM 3.9.0)
// AppleClang  8.1.0  __apple_build_version__ ==  8020041  gsl_COMPILER_APPLECLANG_VERSION ==  810
// (Xcode 8.3.1)               (LLVM 3.9.0) AppleClang  8.1.0  __apple_build_version__ ==  8020042
// gsl_COMPILER_APPLECLANG_VERSION ==  810  (Xcode 8.3.2, 8.3.3)        (LLVM 3.9.0)
// AppleClang  9.0.0  __apple_build_version__ ==  9000037  gsl_COMPILER_APPLECLANG_VERSION ==  900
// (Xcode 9.0)                 (LLVM 4.0.0?) AppleClang  9.0.0  __apple_build_version__ ==  9000038
// gsl_COMPILER_APPLECLANG_VERSION ==  900  (Xcode 9.1)                 (LLVM 4.0.0?)
// AppleClang  9.0.0  __apple_build_version__ ==  9000039  gsl_COMPILER_APPLECLANG_VERSION ==  900
// (Xcode 9.2)                 (LLVM 4.0.0?) AppleClang  9.1.0  __apple_build_version__ ==  9020039
// gsl_COMPILER_APPLECLANG_VERSION ==  910  (Xcode 9.3, 9.3.1)          (LLVM 5.0.2?)
// AppleClang  9.1.0  __apple_build_version__ ==  9020039  gsl_COMPILER_APPLECLANG_VERSION ==  910
// (Xcode 9.4, 9.4.1)          (LLVM 5.0.2?) AppleClang 10.0.0  __apple_build_version__ == 10001145
// gsl_COMPILER_APPLECLANG_VERSION == 1000  (Xcode 10.0, 10.1)          (LLVM 6.0.1?)
// AppleClang 10.0.1  __apple_build_version__ == 10010046  gsl_COMPILER_APPLECLANG_VERSION == 1001
// (Xcode 10.2, 10.2.1, 10.3)  (LLVM 7.0.0?) AppleClang 11.0.0  __apple_build_version__ == 11000033
// gsl_COMPILER_APPLECLANG_VERSION == 1100  (Xcode 11.1, 11.2, 11.3)    (LLVM 8.0.0?)
...
// Presence of language & library features:
  ...
  gsl_BETWEEN(gsl_COMPILER_CLANG_VERSION, 1, 360) || gsl_COMPILER_APPLECLANG_VERSION
#ifdef __EXCEPTIONS
...
// NOTE: When using SFINAE in gsl-lite, please note that overloads of function templates must always
// use SFINAE with non-type default arguments
//       as explained in https://en.cppreference.com/w/cpp/types/enable_if#Notes. `gsl_ENABLE_IF_()`
//       implements graceful fallback to default type arguments (for compilers that don't support
//       non-type default arguments); please verify that this is appropriate in the given situation,
//       and add additional checks if necessary.
//
//       Also, please note that `gsl_ENABLE_IF_()` doesn't enforce the constraint at all if no
//       compiler/library support is available (i.e. pre-C++11).

#if gsl_HAVE(TYPE_TRAITS) && gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
#if !gsl_BETWEEN(gsl_COMPILER_MSVC_VERSION, \
                 1,                         \
                 140)  // VS 2013 seems to have trouble with SFINAE for default non-type arguments
...
//
// NVCC imposes the restriction that a function annotated `__host__ __device__` cannot call
// host-only or device-only functions. This makes `gsl_api` inappropriate for generic functions that
// call unknown code, e.g. the template constructors of `span<>` or functions like `finally()` which
// accept an arbitrary  function object. It is often preferable to annotate functions only with
// `gsl_constexpr` or `gsl_constexpr14`. The "extended constexpr" mode of NVCC (currently an
// experimental feature) will implicitly consider constexpr functions `__host__ __device__`
// functions but tolerates calls to host-only or device-only functions.
...
// add_const<>, add_pointer<>, common_type<>, make_signed<>, remove_cv<>, remove_const<>,
// remove_volatile<>, remove_reference<>, remove_cvref<>, remove_pointer<>, underlying_type<>,
// is_assignable<>, is_constructible<>, is_const<>, is_convertible<>, is_integral<>, is_pointer<>,
// is_signed<>, integral_constant<>, declval()
...
// - C26432: gsl::c.21 : if you define or delete any default operation in the type '...', define or
// delete them all
  ...
  namespace gsl
{
  ...
  // We could try to define `underlying_type<>` for pre-C++11 here, but let's not until someone
  // actually needs it.
  ...
  {
    using R = typename std::common_type<std::ptrdiff_t,
                                        typename std::make_signed<decltype(c.size())>::type>::type;
  ...
  template <class C>
  struct is_container
    : std17::bool_constant<!is_span<C>::value && !is_array<C>::value && !is_std_array<C>::value &&
...
#else  // ^^^ gsl_CPP11_140 && ! gsl_BETWEEN( gsl_COMPILER_GNUC_VERSION, 1, 500 ) ^^^ / vvv !
       // gsl_CPP11_140 || gsl_BETWEEN( gsl_COMPILER_GNUC_VERSION, 1, 500 ) vvv
...
#define gsl_ASSUME(x)                                                        \
  gsl_ELIDE_CONTRACT_(x) /* there is no assume intrinsic in CUDA device code \
                          */
#elif gsl_COMPILER_MSVC_VERSION
...
#else
#define gsl_ASSUME(x)                                                         \
  gsl_ELIDE_CONTRACT_(x) /* unknown compiler; cannot rely on assume intrinsic \
                          */
#endif
...
#define gsl_CONTRACT_CHECK_(str, x) \
  ((x) ? static_cast<void>(0)       \
       : ::gsl::fail_fast_assert_handler(#x, "GSL: " str, __FILE__, __LINE__))
#elif defined(__CUDACC__) && defined(__CUDA_ARCH__)
#define gsl_CONTRACT_CHECK_(str, x) assert((x) && str)
#elif defined(gsl_CONFIG_CONTRACT_VIOLATION_THROWS)
#define gsl_CONTRACT_CHECK_(str, x) \
  ((x) ? static_cast<void>(0)       \
       : ::gsl::detail::fail_fast_throw("GSL: " str " at " __FILE__ ":" gsl_STRINGIFY(__LINE__)))
...
#define gsl_STRINGIFY_(x) #x
  ...
  gsl_api void fail_fast_assert_handler(
    char const *const expression, char const *const message, char const *const file, int line);

#if defined(gsl_CONFIG_CONTRACT_VIOLATION_THROWS)

#if gsl_HAVE(EXCEPTIONS)
  gsl_DEPRECATED_MSG(
    "don't call gsl::fail_fast_assert() directly; use contract checking macros instead")
  ...
  gsl_DEPRECATED_MSG(
    ...
    fail_fast_assert(bool cond,
                     char const *const expression,
                     char const *const message,
                     char const *const file,
                     int line)
  {
    if (!cond) ::gsl::fail_fast_assert_handler(expression, message, file, line);
  }
    ...
    narrow(U u)
  {
    ...
    gsl_SUPPRESS_MSVC_WARNING(
      4127, "conditional expression is constant") if (!detail::is_same_signedness<T, U>::value &&
                                                      (t < T()) != (u < U()))
#endif
#else
...
// helper class to figure out the pointed-to type of a pointer
  ...
  struct not_null_data<T, false> {
    ...
    gsl_constexpr14 not_null_data(T &&_ptr) gsl_noexcept : ptr_(std::move(_ptr)) {}

    gsl_constexpr14 not_null_data(not_null_data &&other) gsl_noexcept : ptr_(std::move(other.ptr_))
    {
    }
  ...
  struct not_null_data<T, true> {
    ...
    gsl_constexpr14 not_null_data(not_null_data const &other) : ptr_(other.ptr_)
    {
...
#if gsl_HAVE(RVALUE_REFERENCE)
    template <class U
    // In Clang 3.x, `is_constructible<not_null<unique_ptr<X>>, unique_ptr<X>>` tries to instantiate
    // the copy constructor of `unique_ptr<>`, triggering an error. Note that Apple Clang's
...
#endif
              >
    gsl_constexpr14 explicit not_null(U other) : data_(T(std::move(other)))
    {
...
#else   // a.k.a. !( gsl_HAVE( TYPE_TRAITS ) && gsl_HAVE( DEFAULT_FUNCTION_TEMPLATE_ARG ) &&
        // !gsl_BETWEEN( gsl_COMPILER_CLANG_VERSION, 1, 400 ) && !gsl_BETWEEN(
        // gsl_COMPILER_APPLECLANG_VERSION, 1, 1001 )
    // If type_traits are not available, then we can't distinguish `is_convertible<>` and
    // `is_constructible<>`, so we unconditionally permit implicit construction.
    template <class U>
    gsl_constexpr14 not_null(U other) : data_(T(std::move(other)))
    {
      gsl_Expects(data_.ptr_ != gsl_nullptr);
    }
#endif  // gsl_HAVE( TYPE_TRAITS ) && gsl_HAVE( DEFAULT_FUNCTION_TEMPLATE_ARG ) && !gsl_BETWEEN(
        // gsl_COMPILER_CLANG_VERSION, 1, 400 ) && !gsl_BETWEEN( gsl_COMPILER_APPLECLANG_VERSION, 1,
        // 1001 )
#else   // a.k.a. !gsl_HAVE( RVALUE_REFERENCE )
    template <class U>
  ...
  !gsl_BETWEEN(gsl_COMPILER_APPLECLANG_VERSION, 1, 1001)
    template <class U
              ...
              typename std::enable_if<(std::is_constructible<T, U>::value &&
                                       !std::is_convertible<U, T>::value),
                                      int>::type = 0>
    gsl_constexpr14 explicit not_null(not_null<U> other) : data_(T(std::move(other.data_.ptr_)))
    {
      gsl_Expects(data_.ptr_ != gsl_nullptr);
    }
    ...
    template <class U
              ...
              ,
              typename std::enable_if<(std::is_convertible<U, T>::value), int>::type = 0>
    gsl_constexpr14 not_null(not_null<U> other) : data_(T(std::move(other.data_.ptr_)))
    {
      gsl_Expects(data_.ptr_ != gsl_nullptr);
    }
#else   // a.k.a. !( gsl_HAVE( TYPE_TRAITS ) && gsl_HAVE( DEFAULT_FUNCTION_TEMPLATE_ARG ) &&
...
#else   // a.k.a. !gsl_HAVE( RVALUE_REFERENCE )
    template <class U>
    gsl_constexpr14 not_null(not_null<U> const &other) : data_(T(other.data_.ptr_))
    {
      gsl_Expects(data_.ptr_ != gsl_nullptr);
    }
    template <class U>
  ...
  gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG) && gsl_HAVE(EXPLICIT)
...
#endif
#else   // a.k.a. #if !( gsl_HAVE( RVALUE_REFERENCE ) && gsl_HAVE( TYPE_TRAITS ) && gsl_HAVE(
    ...
    gsl_constexpr14 not_null(not_null &&other) gsl_noexcept : data_(std::move(other.data_))
    {
      gsl_Expects(data_.ptr_ != gsl_nullptr);
    }
...
//
  ...
  gsl_api inline gsl_constexpr byte operator~(byte b) gsl_noexcept { return ::gsl::to_byte(~b.v); }
  ...
  const gsl_constexpr with_container_t
    with_container;  // TODO: this can lead to ODR violations because the symbol will be defined in
  ...
  class span {
   ...
   public:
...
#else
    template <
      size_t N gsl_ENABLE_IF_((std::is_convertible<value_type (*)[], element_type (*)[]>::value))>
    ...
    template <class U, size_t N>
    gsl_constexpr span(std::array<U, N> &arr) : first_(arr.data()), last_(arr.data() + N)
    {
    }

    template <class U, size_t N>
    gsl_constexpr span(std::array<U, N> const &arr) : first_(arr.data()), last_(arr.data() + N)
    {
    }
    ...
    template <
      size_t N gsl_ENABLE_IF_((std::is_convertible<value_type (*)[], element_type (*)[]>::value))>
    gsl_constexpr span(std::array<value_type, N> &arr) : first_(arr.data()), last_(arr.data() + N)
    {
    }
    ...
    gsl_constexpr span(Container &cont) gsl_noexcept : first_(std17::data(cont)),
    ...
    gsl_constexpr17 reverse_iterator rbegin() const gsl_noexcept { return reverse_iterator(end()); }

    gsl_constexpr17 reverse_iterator rend() const gsl_noexcept { return reverse_iterator(begin()); }
  ...
  template <class T, class U>
  gsl_SUPPRESS_MSGSL_WARNING(stl .1) inline gsl_constexpr bool operator==(span<T> const &l,
                                                                          span<U> const &r)
  {
  ...
  gsl_api gsl_constexpr14 inline OI copy_n(II first, N count, OI result)
  {
    if (count > 0) {
      *result++ = *first;
      for (N i = 1; i < count; ++i) { *result++ = *++first; }
    }
    return result;
  }
  }  // namespace detail
  ...
  template <class Container>
  inline gsl_constexpr span<typename Container::value_type> make_span(with_container_t,
  ...
  class basic_string_span {
   public:
    ...
    template <class Container gsl_ENABLE_IF_(
      (!detail::is_std_array<Container>::value && !detail::is_basic_string_span<Container>::value &&
    ...
    template <class Container>
    gsl_constexpr basic_string_span(with_container_t, Container &cont) : span_(with_container, cont)
    {
    }
#endif
    ...
    gsl_constexpr basic_string_span(
      std::basic_string<typename std11::remove_const<element_type>::type, CharTraits, Allocator>
        &str)
...
#if gsl_HAVE(IS_DEFAULT)
    gsl_constexpr14 basic_string_span &operator=(basic_string_span const &rhs)
    ...
    gsl_constexpr14 basic_string_span first(index_type count) const { return span_.first(count); }

    /*gsl_api*/  // currently disabled due to an apparent NVCC bug
    gsl_constexpr14 basic_string_span last(index_type count) const { return span_.last(count); }
    ...
    gsl_api gsl_constexpr14 reference operator[](index_type idx) const { return span_[idx]; }
    ...
    gsl_api gsl_constexpr14 reference front() const { return span_.front(); }
    ...
    gsl_api gsl_constexpr iterator begin() const gsl_noexcept { return span_.begin(); }

    gsl_api gsl_constexpr iterator end() const gsl_noexcept { return span_.end(); }

    gsl_constexpr17 reverse_iterator rbegin() const gsl_noexcept { return span_.rbegin(); }

    gsl_constexpr17 reverse_iterator rend() const gsl_noexcept { return span_.rend(); }

    // const version not in p0123r2:

    gsl_api gsl_constexpr const_iterator cbegin() const gsl_noexcept { return span_.cbegin(); }

    gsl_api gsl_constexpr const_iterator cend() const gsl_noexcept { return span_.cend(); }

    gsl_constexpr17 const_reverse_iterator crbegin() const gsl_noexcept { return span_.crbegin(); }

    gsl_constexpr17 const_reverse_iterator crend() const gsl_noexcept { return span_.crend(); }
...
inline std::basic_string< typename std::remove_const<T>::type > to_string( basic_string_span<T> spn )
{
  ...
  template <class Container>
  inline span<typename std::remove_pointer<typename Container::pointer>::type> ensure_z(Container &
                                                                                        cont)
  {
  ...
  class basic_zstring_span {
   public:
    ...
    gsl_api gsl_constexpr czstring_type assume_z() const gsl_noexcept { return span_.data(); }
...
// gsl-lite currently keeps all symbols in the namespace `gsl`. The `gsl_lite` namespace contains
// all the symbols in the `gsl` namespace, plus some extensions that are not specified in the Core
// Guidelines.
//
// Going forward, we want to support coexistence of gsl-lite with M-GSL, so we want to encourage
// using the `gsl_lite` namespace when consuming gsl-lite. Typical use in library code would be:
//
...
//     } // namespace foo
//
// In a future version, the new <gsl-lite/gsl-lite.hpp> header will only define the `gsl_lite`
// namespace and no unprefixed `Expects()` and `Ensures()` macros to avoid collision with M-GSL. To
// ensure backward compatibility, the old header <gsl/gsl-lite.hpp> will keep defining the `gsl`
// namespace and the `Expects()` and `Ensures()` macros.
