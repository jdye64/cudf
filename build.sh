...
VALIDARGS="clean libcudf cudf dask_cudf custreamz external benchmarks tests -v -g -n --allgpuarch --disable_nvtx --show_depr_warn -h"
HELP="$0 [clean] [libcudf] [cudf] [dask_cudf] [custreamz] [external] [benchmarks] [tests] [-v] [-g] [-n] [-h]
...
BUILD_DIRS="${LIB_BUILD_DIR} ${CUDF_BUILD_DIR} ${DASK_CUDF_BUILD_DIR} ${CUSTREAMZ_BUILD_DIR} ${EXTERNAL_BUILD_DIR}"
