...
class MockKafkaConsumer : public RdKafka::KafkaConsumer {
 public:
  ...
  MOCK_METHOD4(
    metadata, RdKafka::ErrorCode(bool, const RdKafka::Topic*, RdKafka::Metadata**, int timeout_ms));
...
class MockConsumer : public Consumer {
 public:
  MockConsumer(std::string brokerList,
               std::string topicStr,
               int partition,
               MockKafkaTopic* kafkaTopic,
               MockKafkaConsumer* kafkaConsumer)
    : infra::kafka::Consumer(
        brokerList, topicStr, partition, "infra-kafka-consumer-test", "test-key", false, nullptr),
 ...
 protected:
  std::unique_ptr<RdKafka::Topic> createKafkaTopic(RdKafka::KafkaConsumer* consumer,
  ...
  std::unique_ptr<RdKafka::KafkaConsumer> createKafkaConsumer(RdKafka::Conf* conf,
                                                              std::string* errstr) override
  {
